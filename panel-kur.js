...
const KOY9 = "\n/* WHAPI PANEL GORUNUMU — TOPLU HAT DEGISTIRME\n   Listedeki TUM kullanicilari tek seferde Whapi'ye alir ya da Baileys'e\n   dondurur. Sunucu her birinin oturumunu BELLEKTE tazeliyor ve acik\n   panellere 'hatDegisti' yolluyor -> o kisiler F5 ATMADAN yenileniyor. */\nasync function whapiHepsiniDegistir(whapiYap){\n  const hedef = whapiYap ? 'WHAPI' : 'BAILEYS (ofis)';\n  const liste = (typeof _userListesi!=='undefined' && Array.isArray(_userListesi)) ? _userListesi : null;\n  if(!liste || !liste.length){ alert('Kullanici listesi okunamadi. Once Kullanici Yonetimi ekranini ac.'); return; }\n  const adlar = liste.map(function(u){ return u.username; }).filter(Boolean);\n  if(!confirm(adlar.length+' kullanicinin HEPSI '+hedef+' hattina alinacak.\\n\\nDevam?')) return;\n  var ok=0, hata=0, hataliAdlar=[];\n  for (var i=0;i<adlar.length;i++){\n    try{\n      const r = await fetch('/api/users/setline',{method:'POST',\n        headers:{'Content-Type':'application/json'},\n        body: JSON.stringify({token:authToken, username:adlar[i], tip: whapiYap?'whapi':'ofis'})});\n      const d = await r.json();\n      if(d && d.ok) ok++; else { hata++; hataliAdlar.push(adlar[i]); }\n    }catch(e){ hata++; hataliAdlar.push(adlar[i]); }\n  }\n  if(typeof userListeCiz==='function') userListeCiz();\n  var mesaj = ok+' kullanici '+hedef+' hattina alindi.';\n  if(hata) mesaj += '\\n'+hata+' tanesi olmadi: '+hataliAdlar.slice(0,8).join(', ');\n  mesaj += '\\n\\nAcik panelleri olanlar OTOMATIK yenilenir.';\n  alert(mesaj);\n  const ben = (typeof currentUser!=='undefined' && currentUser && currentUser.username) || '';\n  if(ben && adlar.indexOf(ben)>-1) setTimeout(function(){ location.reload(); }, 900);\n}\n/* WHAPI PANEL GORUNUMU — hat degistirme (yonetici) */\nasync function whapiHatDegistir(username, whapiYap){\n  const hedef = whapiYap ? 'WHAPI' : 'BAILEYS (ofis)';\n  if(!confirm(username+' kullanicisi '+hedef+' hattina alinacak.\\n\\nO kisi ACIKSA CIKIP TEKRAR GIRMELI — sayfa yenilemek YETMEZ.\\n\\nDevam?')) return;\n  try{\n    const r = await fetch('/api/users/setline',{\n      method:'POST', headers:{'Content-Type':'application/json'},\n      body: JSON.stringify({ token: authToken, username: username, tip: whapiYap ? 'whapi' : 'ofis' })\n    });\n    const d = await r.json();\n    if(!d.ok){ alert('Olmadi: '+(d.error||'bilinmeyen hata')); return; }\n    if(typeof userListeCiz==='function') userListeCiz();\n    const kendim = (typeof currentUser!=='undefined' && currentUser && currentUser.username===username);\n    if(kendim){\n      alert('Sen '+hedef+' hattina alindin.\\n\\nOturumun yenilenmesi icin CIKIS yapiliyor, tekrar giris yap.');\n      whapiOturumuBirak();\n    } else {\n      alert(username+' -> '+hedef+'\\n\\nBu kisi CIKIP TEKRAR GIRMELI.');\n    }\n  }catch(e){ alert('Baglanti hatasi: '+e.message); }\n}\n\n/* Oturumu gercekten birak. Sayfa yenilemek YETMEZ: sunucu oturumun hattini\n   bellekte onbellekliyor (whoami yalnizca lineId HIC yoksa DB'den okuyor).\n   Bu yuzden token'i atip giris ekranina donuyoruz. */\nfunction whapiOturumuBirak(){\n  try{ ws.send(JSON.stringify({type:'logout'})); }catch(e){}\n  try{ authToken=null; sessionStorage.removeItem('pekcan_token'); }catch(e){}\n  setTimeout(function(){ location.reload(); }, 400);\n}\n\n/* Kullanici KENDINI Baileys'e dondurur (hat koptugunda acil cikis yolu).\n   Sunucu bu yonu yetki istemeden kabul eder. */\nasync function whapiBaileyseDon(){\n  if(!confirm('Baileys (ofis) hattina donulecek.\\n\\nCikis yapilip giris ekranina donulecek. Devam?')) return;\n  try{\n    const kim=(typeof currentUser!=='undefined' && currentUser && currentUser.username)||'';\n    if(!kim){ alert('Kullanici bilgisi okunamadi, cikis yapip tekrar gir.'); return; }\n    const r=await fetch('/api/users/setline',{method:'POST',\n      headers:{'Content-Type':'application/json'},\n      body:JSON.stringify({token:authToken,username:kim,tip:'ofis'})});\n    const d=await r.json();\n    if(d.ok){ alert('Baileys hattina donuldu. Tekrar giris yap.'); whapiOturumuBirak(); }\n    else alert('Olmadi: '+(d.error||'bilinmeyen hata'));\n  }catch(e){ alert('Baglanti hatasi: '+e.message); }\n}\nfunction gorevMenuAc(ev, id, suanki){";
...
// ── ON IKINCI YAMA: UYARILARI SUSTUR + HAT DEGISTI SINYALI ──
const ARA12 = "    else if(d.type==='coklisession'){\n      // Ayni kullanici birden fazla yerde acik -> WhatsApp oturumunu bozabilir, uyar.\n      cokliSessionUyar(d.adet);\n    }";
const KOY12 = "    else if(d.type==='coklisession'){\n      /* WHAPI PANEL GORUNUMU — coklu oturum uyarisi KAPATILDI (2026-08)\n         Kullanici istegi: WhatsApp KOPMASI disinda uyari cikmasin.\n         Bu uyari WhatsApp'tan degil, panelin kendisinden geliyordu\n         (ayni kullanici birden fazla sekmede acik). Bilgi olarak\n         konsola yaziliyor, ekrani kapatmiyor. */\n      try{ console.log('[bilgi] bu hesap '+d.adet+' yerde acik'); }catch(e){}\n    }\n    else if(d.type==='hatDegisti'){\n      /* WHAPI PANEL GORUNUMU — hat degisti, F5 GEREKMEDEN yenile.\n         Sunucu oturumu bellekte tazeledi; sayfayi yeniden yukleyince\n         yeni hattin sohbetleri gelir. Cikis YAPILMAZ, token gecerli kalir. */\n      try{\n        showToast && showToast('Hat degisti ('+(d.tip==='whapi'?'WHAPI':'BAILEYS')+') — yenileniyor…');\n      }catch(e){}\n      setTimeout(function(){ location.reload(); }, 700);\n    }";

// ── ON UCUNCU YAMA: SOHBET YENILEME DUGMESINI GIZLE ─────────
const ARA13 = "<button class=\"gd-yenile\" id=\"gdYenileBtn\" onclick=\"sohbetYenile()\"";
const KOY13 = "<button class=\"gd-yenile\" id=\"gdYenileBtn\" style=\"display:none\" onclick=\"sohbetYenile()\"";

// ── ON DORDUNCU YAMA: TOPLU HAT DEGISTIRME DUGMELERI ────────
const ARA14 = "        <p style=\"color:var(--muted);font-size:13px;margin-bottom:18px\">Ekibine kullanıcı aç, rol ver ya da sil. Herkes kendi kullanıcı adı ve şifresiyle girer.</p>";
const KOY14 = "        <p style=\"color:var(--muted);font-size:13px;margin-bottom:18px\">Ekibine kullanıcı aç, rol ver ya da sil. Herkes kendi kullanıcı adı ve şifresiyle girer.</p>\n        <!-- WHAPI PANEL GORUNUMU — TOPLU HAT DEGISTIRME -->\n        <div style=\"display:flex;gap:8px;margin:-8px 0 16px;flex-wrap:wrap\">\n          <button class=\"btn ghost\" style=\"font-size:12.5px;padding:7px 12px\"\n            onclick=\"whapiHepsiniDegistir(true)\">🔁 Herkesi Whapi'ye al</button>\n          <button class=\"btn ghost\" style=\"font-size:12.5px;padding:7px 12px\"\n            onclick=\"whapiHepsiniDegistir(false)\">↩️ Herkesi Baileys'e al</button>\n        </div>";
...
function kur() {
  ...
  const kontrol = [['isim etiketi', ARA], ['balon sinifi', ARA2], ['stil blogu', ARA3], ['gruplama', ARA4], ['sirali ekleme', ARA5], ['ceviri kapatma', ARA6], ['hat dugmesi', ARA8], ['hat fonksiyonu', ARA9], ['acil donus', ARA10], ['donus fonksiyonu', ARA11], ['uyari susturma', ARA12], ['yenile gizle', ARA13], ['toplu dugme', ARA14]];
  ...
  const eslesme = { 'isim etiketi': [ARA, KOY], 'balon sinifi': [ARA2, KOY2],
    ...
    'uyari susturma': [ARA12, KOY12], 'yenile gizle': [ARA13, KOY13], 'toplu dugme': [ARA14, KOY14] };
  ...
  console.log('✔ coklu oturum uyarisi kapatildi + hat degisince otomatik yenileme');
  console.log('✔ sohbet yenileme dugmesi gizlendi');
  console.log('✔ TOPLU hat degistirme dugmeleri eklendi');
  console.log('');
